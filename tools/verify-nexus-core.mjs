...
const controlFetchV2Path = path.join(ROOT, 'packages', 'nexus-core', 'src', 'api', 'control', 'client', 'fetch-v2.ts')
const controlViewAccessPath = path.join(ROOT, 'packages', 'nexus-core', 'src', 'api', 'control', 'client', 'view-access.ts')
const controlCapabilitiesPath = path.join(ROOT, 'packages', 'nexus-core', 'src', 'api', 'control', 'client', 'capabilities.ts')
...
const controlFetchV2Source = readFileSync(controlFetchV2Path, 'utf8')
const controlViewAccessSource = readFileSync(controlViewAccessPath, 'utf8')
const controlCapabilitiesSource = readFileSync(controlCapabilitiesPath, 'utf8')
...
expect(!controlFetchV2Source.includes('AUTH_REQUIRED_SKIPPED'), 'Control API public bundle reads must not skip without auth')
expect(!controlFetchV2Source.includes('!client.token && !client.ingestKey'), 'Control API public bundle reads must reach public GET endpoints without auth')
expect(controlViewAccessSource.includes('AUTH_REQUIRED_SKIPPED'), 'View access validation must stay auth-gated without token or ingest key')
expect(controlCapabilitiesSource.includes('AUTH_REQUIRED_SKIPPED'), 'Capability reporting must stay auth-gated without token or ingest key')
