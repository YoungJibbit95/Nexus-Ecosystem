...
const withAndroid =
  (args.has("--with-android") || args.has("--android")) &&
...
const withControlPlane =
  args.has("--with-control-plane") || args.has("--with-api");
...
const unescapeGradlePath = (value) =>
  String(value || "")
    .replace(/\\:/g, ":")
    .replace(/\\\\/g, "\\");
...
const collectAndroidArtifacts = async (outputRoot) => {
  ...
  return allFiles.filter(
    (file) => file.endsWith(".apk") || file.endsWith(".aab"),
  );
};
...
const collectInstallerArtifacts = async (releaseRoot) => {
  ...
  return allFiles.filter((file) =>
    INSTALLER_EXTENSIONS.has(path.extname(file).toLowerCase()),
  );
};
...
const buildElectronInstallersForApp = async (app, warnings) => {
  ...
  try {
    ...
    if (cached.length > 0) {
      warnings.push(
        `[${app.name}] Vorhandene Installer-Artefakte aus ${app.electron.releaseDir || "release"} werden weiterverwendet.`,
      );
    }
...
const buildAndroidForApp = async (app, warnings) => {
  ...
  if (!sdkPath) {
    ...
    warnings.push(
      `[${app.name}] ANDROID_HOME/ANDROID_SDK_ROOT nicht gesetzt, Android-Build wurde uebersprungen.`,
    );
    return [];
  }
...
const main = async () => {
  ...
  for (const sharedExport of sharedExports) {
    ...
    if (!copied) {
      manifest.warnings.push(
        `${sharedExport.label} unter ${path.relative(ROOT, sharedExport.from)} nicht gefunden.`,
      );
    }
  }
  ...
  for (const app of APPS) {
    ...
    const appWarnings = [];
    const installerBuild = await buildElectronInstallersForApp(
      app,
      appWarnings,
    );

    const needsAppBuild = !(
      installerBuild.attempted && installerBuild.succeeded
    );
    if (needsAppBuild) {
      runNpmCommand(["--prefix", appRoot, "run", app.buildScript], {
        cwd: ROOT,
      });
    }
    ...
    manifest.apps.push({
      ...
      installerArtifacts: installerBuild.installerArtifacts.map((file) =>
        path.relative(ROOT, file),
      ),
      installerBuildAttempted: installerBuild.attempted,
      installerBuildSucceeded: installerBuild.succeeded,
      androidArtifacts: androidArtifacts.map((file) =>
        path.relative(ROOT, file),
      ),
