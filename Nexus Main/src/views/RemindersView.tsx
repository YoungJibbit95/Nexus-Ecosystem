...
export function RemindersView({
  ...
  const [lastRescheduleReason, setLastRescheduleReason] = useState<
    string | null
  >(null);
    ...
    if (search)
      rs = rs.filter(
        (r) =>
          ...
          r.msg?.toLowerCase().includes(search.toLowerCase()),
      );
    switch (filter) {
      case "upcoming":
        return rs
      ...
      case "soon":
        return rs
          .filter(
            (r) =>
              !r.done &&
              new Date(r.snoozeUntil || r.datetime) >= now &&
              new Date(r.snoozeUntil || r.datetime).getTime() - now.getTime() <=
                30 * 60000,
          )
          .sort(
            (a, b) =>
  ...
  const soonCount = upcoming.filter(
    (r) =>
      new Date(r.snoozeUntil || r.datetime).getTime() - now.getTime() <
      30 * 60000,
  ).length;
  ...
  const staleDoneCount = useMemo(() => {
    ...
    return done.filter(
      (reminder) => new Date(reminder.datetime).getTime() < cutoff,
    ).length;
  }, [done]);

  const reminderQuality = useMemo(
    () =>
      calculateNexusViewQuality({
  ...
  const runHealthCheck = useCallback(() => {
    ...
    setControlMsg(
      notifyAvailable
        ? "Notification bridge verfügbar"
        : "Notification bridge nicht verfügbar",
    );
  ...
  const clearDone = useCallback(() => {
    ...
    setControlMsg(
      doneItems.length > 0
        ? "Erledigte Reminder gelöscht"
        : "Keine erledigten Reminder",
    );
    window.setTimeout(() => setControlMsg(""), 2600);
  }, [reminders, delRem]);

  const createTemplateReminder = useCallback(
  ...
  return (
    <div
      className="nx-reminders-v6 nx-release-view"
      style={{
        display: "flex",
    ...
    >
      ...
      >
        ...
        >
          {(["upcoming", "soon", "overdue", "all", "done"] as const).map(
            (f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
      ...
      {/* ── Stats strip ── */}
      <div
        className="nx-reminders-stats nx-release-strip"
        style={{
          display: "flex",
          alignItems: "center",
          gap: 14,
      ...
      >
        {[
          { label: "Upcoming", val: upcoming.length, color: t.accent },
          {
            label: "Overdue",
            val: overdue.length,
            color: overdue.length ? "#ff453a" : "inherit",
          },
          {
            label: "Soon (<30m)",
            val: soonCount,
            color: soonCount ? "#ff9f0a" : "inherit",
          },
      ...
      <div
        className="nx-reminders-today-strip nx-release-strip"
        style={{
          display: "flex",
          alignItems: "center",
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          <button
            ...
            style={{
              ...
              color: controlOpen ? t.accent : "inherit",
              cursor: "pointer",
              fontSize: 11,
              fontWeight: 700,
              display: "inline-flex",
          ...
          <Glass style={{ padding: "10px 12px" }}>
              ...
              >
                <span
                  style={{
                    ...
                    padding: "3px 8px",
                    borderRadius: 999,
                    border: notifyAvailable
                      ? "1px solid rgba(48,209,88,0.35)"
                      : "1px solid rgba(255,69,58,0.35)",
                    background: notifyAvailable
                      ? "rgba(48,209,88,0.12)"
                      : "rgba(255,69,58,0.12)",
                    color: notifyAvailable ? "#30d158" : "#ff453a",
                    fontWeight: 700,
                  }}
                >
            ...
            >
              ...
              <span style={{ fontSize: 11, opacity: 0.72 }}>
                ...
                <b>
                  {lastRescheduleAt
                    ? new Date(lastRescheduleAt).toLocaleString()
                    : "noch keiner"}
                </b>
              </span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              <button
                onClick={() =>
                  setControlMsg(
                    "Fallback: lokaler Polling-Checker läuft alle 15s.",
                  )
                }
                style={{
                  padding: "6px 10px",
              ...
              >
                ...
                {lastRescheduleReason ? (
                  <span style={{ fontSize: 10, opacity: 0.62 }}>
                    Reschedule-Status: {lastRescheduleReason}
                  </span>
                ) : null}
                {lastHealthCheckAt ? (
                  <span style={{ fontSize: 10, opacity: 0.62 }}>
                    Health Check: {new Date(lastHealthCheckAt).toLocaleString()}
                  </span>
                ) : null}
              </div>
            ) : null}
