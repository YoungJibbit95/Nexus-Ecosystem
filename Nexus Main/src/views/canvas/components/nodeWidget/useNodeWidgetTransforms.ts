...
export const useNodeWidgetTransforms = ({
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const rawX = dragStart.current.nodeX + (e.clientX - dragStart.current.x) / zoom;
      const rawY = dragStart.current.nodeY + (e.clientY - dragStart.current.y) / zoom;
      const grid = 24;
  ...
  }, [dragging, dragPreview, moveNode, node.id, snapToGrid]);
