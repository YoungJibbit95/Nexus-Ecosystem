...
export function SurfaceBoundsDebugOverlay({
  ...
  React.useEffect(() => {
    ...
    const refresh = () => {
      if (!active) return;
      setRects(collectOverlayRects(surfaces.slice(0, 64)));
    };

    refresh();
    const interval = window.setInterval(refresh, 320);
      ...
      {rects.map((entry) => {
        ...
        return (
          <React.Fragment key={entry.id}>
            ...
            >
              {entry.id} · {entry.mode} · {entry.motionCapability} · area {entry.areaHint}
            </div>
          </React.Fragment>
        );
      })}
