...
export const RemindersView = lazy(() => loadRemindersView().then(m => ({ default: m.RemindersView })))
export const CanvasView = lazy(() => loadCanvasView().then(m => ({ default: m.CanvasView })))
export const FilesView = lazy(() => loadFilesView().then(m => ({ default: m.FilesView })))
export const FluxView = lazy(() => loadFluxView().then(m => ({ default: m.FluxView })))
...
const runLoaderOnce = (loader: () => Promise<unknown>) => {
  const existing = MOBILE_PRELOAD_PROMISES.get(loader)
  if (existing) return existing
  const next = loader().catch((error) => {
    MOBILE_PRELOAD_PROMISES.delete(loader)
    throw error
  })
  MOBILE_PRELOAD_PROMISES.set(loader, next)
  return next
}
...
export const orderMobilePreloadViews = (views: View[]) => {
...
export const preloadMobileViews = async (
  views: View[],
  options: {
    eagerLimit?: number
  ...
  const loaders = orderMobilePreloadViews(views)
    .filter((viewId) => allowHeavy || !MOBILE_HEAVY_PRELOAD_VIEWS.has(viewId))
