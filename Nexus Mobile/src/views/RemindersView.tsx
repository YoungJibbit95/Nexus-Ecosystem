...
function ReminderModal({ reminder, onClose }: { reminder?: Reminder; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      onClick={onClose}>
      <motion.div initial={{scale:0.92,y:20}} animate={{scale:1,y:0}} exit={{scale:0.92,y:20}} onClick={e=>e.stopPropagation()} style={{ width: mob.isMobile ? '100%' : 440, maxHeight: mob.isMobile ? '90vh' : '85vh', display:'flex', flexDirection:'column', borderRadius: mob.isMobile ? '20px 20px 0 0' : undefined, position: mob.isMobile ? 'fixed' : undefined, bottom: mob.isMobile ? 0 : undefined, left: mob.isMobile ? 0 : undefined, right: mob.isMobile ? 0 : undefined }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'rgba(0,0,0,0.05)', flexWrap:'wrap' }}>
        <span style={{ fontSize:10, opacity:0.55, fontWeight:700, letterSpacing:0.4, textTransform:'uppercase' }}>Quick flows</span>
        <button onClick={() => quickReminder('standup')} style={{ padding:'5px 9px', borderRadius:8, border:`1px solid rgba(${rgb},0.3)`, background:`rgba(${rgb},0.14)`, color:t.accent, fontSize:10, fontWeight:700, cursor:'pointer' }}>Daily Standup</button>
        <button onClick={() => quickReminder('followup')} style={{ padding:'5px 9px', borderRadius:8, border:'1px solid rgba(255,255,255,0.14)', background:'rgba(255,255,255,0.06)', color:'inherit', fontSize:10, fontWeight:700, cursor:'pointer' }}>Follow-up +2h</button>
        <button onClick={() => quickReminder('break')} style={{ padding:'5px 9px', borderRadius:8, border:'1px solid rgba(255,255,255,0.14)', background:'rgba(255,255,255,0.06)', color:'inherit', fontSize:10, fontWeight:700, cursor:'pointer' }}>Break +45m</button>
      </div>
