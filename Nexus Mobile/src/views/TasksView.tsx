  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
        ...
        <div style={{ flex:1, display: mob.isMobile ? 'block' : 'flex', gap:12, padding: mob.isMobile ? '0 12px 12px' : 14, overflow:'hidden', minHeight:0, overflowY: mob.isMobile ? 'auto' : undefined }}>
          {COLS.filter(col => !mob.isMobile || col.id === mobileCol).map(col => {
            ...
            return (
              <div key={col.id} style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, minHeight:0 }}>
                ...
                {/* Cards */}
                <div style={{ flex: mob.isMobile ? undefined : 1, overflowY: mob.isMobile ? 'visible' : 'auto', paddingRight:2 }}>
                  <DropCol id={col.id}>
                    {items.length === 0 ? (
