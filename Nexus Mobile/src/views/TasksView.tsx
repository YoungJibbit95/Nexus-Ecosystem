            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8 }}>
                ...
                <div style={{ display:'flex', gap:3, flexShrink:0 }}>
                  <InteractiveIconButton motionId={`mobile-task-edit-${tk.id}`} onClick={onEdit} idleOpacity={0.35} radius={4}>
                    <Edit3 size={11}/>
                  </InteractiveIconButton>
                  <InteractiveIconButton motionId={`mobile-task-delete-${tk.id}`} onClick={onDelete} intent="danger" idleOpacity={0.35} radius={4}>
                    <Trash2 size={11}/>
                  </InteractiveIconButton>
                </div>
              </div>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:6, marginTop:7 }}>
                ...
                {linkedNoteTitle && (
                  <button onClick={() => onOpenLinkedNote?.(tk)} style={{ display:'flex', alignItems:'center', gap:3, fontSize:10, padding:'2px 7px', borderRadius:10, background:`rgba(${rgb},0.12)`, color:t.accent, border:`1px solid rgba(${rgb},0.2)`, cursor:'pointer', maxWidth:160 }} title={linkedNoteTitle}>
                    <Link size={9} />
              ...
              {mob.isMobile && !batchMode && (
                <div style={{ display:'flex', gap:6, marginTop:8 }}>
                  {COLS.map((column) => {
                    const active = tk.status === column.id
                    return (
                      <button
                        key={`${tk.id}-${column.id}`}
...
function TaskModal({
  ...
  const modalDecision = useRenderSurfaceBudget({
    id: `mobile-tasks-modal-${task?.id ?? status ?? 'new'}`,
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'14px 20px' }}>
                {task && (task.subtasks||[]).map((subtask) => (
                  <div key={subtask.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', marginBottom:4, borderRadius:8, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <InteractiveIconButton motionId={`mobile-task-sub-delete-${task.id}-${subtask.id}`} intent="danger" idleOpacity={0.4} radius={4} style={{ padding: 2 }} onClick={() => task && updateTask(task.id,{subtasks:task.subtasks.filter((entry) => entry.id !== subtask.id)})}>
                      <Trash2 size={11}/>
                    </InteractiveIconButton>
                  </div>
                ))}
            ...
            return (
              <div key={column.id} style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, minHeight:0 }}>
                <Glass style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', marginBottom:8, flexShrink:0 }}>
                  ...
                  <InteractiveIconButton motionId={`mobile-task-col-add-${column.id}`} intent="accent" idleOpacity={0.7} radius={6} style={{ padding: '3px 5px' }} onClick={() => setNewStatus(column.id)}>
                    <Plus size={15}/>
                  </InteractiveIconButton>
                </Glass>
