...
export function FilesView() {
  ...
  const filtered = useMemo(() => {
    ...
    return items
  }, [allItems, typeFilter, search, onlyUnassigned, activeWorkspaceId, tab, workspaces])
    ...
    <div style={{ display:'flex', flex:1, minHeight:0, overflow:'hidden' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:'rgba(0,0,0,0.05)', flexWrap:'wrap' }}>
          ...
          <button onClick={() => setOnlyUnassigned(s => !s)} style={{ padding:'5px 9px', borderRadius:8, border:`1px solid ${onlyUnassigned ? t.accent : 'rgba(255,255,255,0.14)'}`, background:onlyUnassigned ? `rgba(${rgb},0.14)` : 'rgba(255,255,255,0.06)', color:onlyUnassigned ? t.accent : 'inherit', fontSize:10, fontWeight:700, cursor:'pointer' }}>
