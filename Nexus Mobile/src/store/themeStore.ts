...
const sanitizeThemeSnapshot = (persistedRaw: unknown, current: Theme): Theme => {
  ...
  return {
    ...
    qol: mergedQol,
    tokens: buildTokens(
      persisted.mode === 'light' ? 'light' : 'dark',
      typeof persisted.accent === 'string' && persisted.accent.length > 0 ? persisted.accent : current.accent,
      Boolean(autoAccentContrast),
    ),
...
export const useTheme = create<Theme>()(
  persist(
    (set) => ({
      ...
      setQOL: (q) => set((s) => {
        const nextQol = sanitizeQolConfig(q, s.qol)
        return {
          qol: nextQol,
