...
const vec3 NEXUS_VOID = vec3(2.0, 6.0, 23.0) / 255.0;
const vec3 NEXUS_BLUE = vec3(37.0, 99.0, 235.0) / 255.0;
const vec3 NEXUS_CYAN = vec3(34.0, 211.0, 238.0) / 255.0;
...
vec3 backgroundColor(vec2 uv) {
  float ridge = sin(uv.x * 0.9 - 0.16) * 0.25;
  float m = uv.y - ridge;

  vec3 col = vec3(0.0);
  col += mix(NEXUS_BLUE, NEXUS_VOID, smoothstep(0.0, 1.0, abs(m)));
  col += mix(NEXUS_CYAN, NEXUS_VOID, smoothstep(0.0, 1.0, abs(m - 0.75)));
  return col * 0.48;
}
...
const FALLBACK_GRADIENT = ["#67e8f9", "#22d3ee", "#818cf8", "#6366f1", "#a78bfa"];
...
export default function FloatingLines({
  linesGradient = FALLBACK_GRADIENT,
  ...
  const topLineCount = clamp(enabledWaves.includes("top") ? getLineCount("top") : 0, 0, 24);
  const middleLineCount = clamp(enabledWaves.includes("middle") ? getLineCount("middle") : 0, 0, 24);
  const bottomLineCount = clamp(enabledWaves.includes("bottom") ? getLineCount("bottom") : 0, 0, 24);
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      topWavePosition: {
        value: new Vector3(topWavePosition?.x ?? 9.2, topWavePosition?.y ?? 0.5, topWavePosition?.rotate ?? -0.3),
      },
      middleWavePosition: {
        value: new Vector3(middleWavePosition?.x ?? 4.8, middleWavePosition?.y ?? 0.0, middleWavePosition?.rotate ?? 0.18),
      },
      bottomWavePosition: {
        value: new Vector3(bottomWavePosition?.x ?? 2.0, bottomWavePosition?.y ?? -0.7, bottomWavePosition?.rotate ?? 0.32),
      },
