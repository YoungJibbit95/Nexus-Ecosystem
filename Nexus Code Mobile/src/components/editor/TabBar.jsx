...
export default function TabBar({ tabs, activeTabId, onTabSelect, onTabClose }) {
  ...
  const tabHeight = isMobile ? "h-10" : "h-9";

  return (
    <div
      className={`flex items-center ${tabHeight} overflow-x-auto shrink-0`}
      style={{
          ...
          return (
            <motion.div
              ...
              className={`flex items-center gap-2 px-3 h-full cursor-pointer group relative shrink-0
                ${isMobile ? "min-w-[120px] max-w-[180px]" : ""}`}
              style={{
            ...
            >
              ...
              {/* Close button — always visible on mobile, hover-only on desktop */}
              <motion.button
