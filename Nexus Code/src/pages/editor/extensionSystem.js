import {
  ...
  describeActivationEvent,
  EXTENSION_RUNTIME_CONTRIBUTION_POINTS,
  formatContributionPreview,
  getExtensionContributionItemLabel,
  language,
...
const createDiagnostic = createExtensionDiagnostic;
...
const createManifest = ({
  ...
  const normalizedContributes = normalizeExtensionContributes(contributes);
  const manifest = {
    manifestVersion: 1,
    name,
...
export const EXTENSION_CATALOG = [
  ...
  createManifest({
    ...
    contributes: {
      ...
      keybindings: [{ command: "prettier.formatDocument", key: "shift+alt+f" }],
  ...
  createManifest({
    ...
    contributes: {
      ...
      keybindings: [{ command: "eslint.fixAll", key: "ctrl+shift+.", mac: "cmd+shift+." }],
      languages: [
        language("javascript", ["JavaScript"], [".js", ".jsx"]),
        language("typescript", ["TypeScript"], [".ts", ".tsx"]),
      ],
...
export function getExtensionStats(records) {
  ...
  return {
    ...
    keybindings: contributions.keybindings?.length || 0,
    snippets: contributions.snippets?.length || 0,
