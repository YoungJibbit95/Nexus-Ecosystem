...
const bootstrap = async () => {
  ...
  try {
    ...
    await core.runBootstrapHandshake({ force: true })
    core.setStatus(`Eingeloggt als ${state.session.username} (${state.session.role})`, 'success')
  } catch (error) {
    core.setStatus(`Daten konnten nicht geladen werden: ${parseErrorMessage(error)}`, 'error')
  }
}
