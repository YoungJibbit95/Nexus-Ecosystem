...
const shutdown = (signal) => {
  ...
  shuttingDown = true
  console.log(`\nControl UI (dev) shutdown via ${signal}`)
...
server.on('error', (error) => {
  if (error?.code === 'EADDRINUSE') {
    console.error(`Control UI (dev) Start fehlgeschlagen: Port ${PORT} bereits belegt`)
    process.exit(1)
  }
  console.error('Control UI (dev) Server Fehler', error)
  process.exit(1)
})
...
try {
  ...
  console.error(`Control UI (dev) Start fehlgeschlagen: ${error?.message || 'Port nicht verfuegbar'}`)
  process.exit(1)
}

server.listen(PORT, HOST, () => {
  console.log(`Nexus Control UI (dev) auf http://127.0.0.1:${PORT}`)
})
